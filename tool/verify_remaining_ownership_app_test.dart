// Exercises isolated candidate data through the production placed-agent widget.
import 'dart:convert';
import 'dart:io';
...
class _DeliveredAssets extends SvgHeightRuntimeDependencies {
  ...
  Future<Uint8List> _load(String asset) async {
    ...
    expect(bytes, await File(candidate).readAsBytes(),
        reason:
            'Isolated candidate provider loads exact frozen candidate bytes; final bundle proof remains central.');
    return bytes;
  }
...
void main() {
  ...
  testWidgets(
      'isolated $_mapName candidate survives placement, side flip and drag',
      (tester) async {
    final completed = await tester.runAsync(() async {
      ...
      for (final path in [
        ...
        'tool/verify_remaining_ownership_app_test.dart',
      ]) {
      ...
      File('${output.path}/verification.json')
        ...
        'sourceFixtureSha256': fixtureHash.bytes
            ...
            .join(),
        'surface':
            'Flutter widget integration using isolated candidate files and production provider, placed agent, cache and painter. The loader reads frozen candidate models instead of the delivered bundle. No live-game certification.',
