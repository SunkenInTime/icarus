...
struct LocalSyncState {
    ...
    last_auth_token: AuthenticationToken,
}
...
impl LocalSyncState {
    ...
    fn authenticate(&mut self, token: AuthenticationToken) -> (ClientMessage, bool) {
        let base_version = self.identity_version;
        self.identity_version += 1;
        let token_changed = token != self.last_auth_token;
        self.last_auth_token = token.clone();
        (
    ...
    async fn restart(&mut self) -> (Vec<ClientMessage>, bool) {
        self.identity_version = 0;
        let mut messages = Vec::new();
        let mut auth_token_changed = false;
        ...
        if let Some(ref fetcher) = self.auth_fetcher {
            match fetcher(true).await {
                Ok(token) if token != AuthenticationToken::None => {
                    auth_token_changed = token != self.last_auth_token;
                    self.last_auth_token = token.clone();
...
impl BaseConvexClient {
    ...
    pub async fn set_auth_fetcher(&mut self, fetcher: Option<AuthTokenFetcher>) -> bool {
        let mut auth_token_changed = false;
        match fetcher {
            Some(fetcher) => {
                match fetcher(false).await {
                    Ok(token) => {
                        let (message, changed) = self.state.authenticate(token);
                        auth_token_changed = changed;
                        self.outgoing_message_queue.push_back(message);
                    },
                    Err(e) => {
            ...
            None => {
                ...
                let (message, changed) = self.state.authenticate(AuthenticationToken::None);
                auth_token_changed = changed;
                self.outgoing_message_queue.push_back(message);
            },
        }
    ...
    pub async fn resend_ongoing_queries_mutations(&mut self) -> bool {
        ...
        let (state_restart_messages, auth_token_changed) = self.state.restart().await;
...
mod tests {
    ...
    async fn test_reconnect_does_not_send_duplicate_version_messages() {
        ...
        assert!(!client.resend_ongoing_queries_mutations().await);
