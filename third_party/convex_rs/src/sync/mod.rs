...
pub struct ReconnectRequest {
    ...
    /// The old socket failed while recovering a server auth rejection. The
    /// client worker already paces these retries, so the WebSocket layer must
    /// not add its independent network backoff.
    pub auth_retry: bool,
}
