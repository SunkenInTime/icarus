...
impl WebSocketWorker {
    ...
    async fn run(
        ...
        loop {
            ...
            let mut delay = worker.backoff.fail(&mut rand::rng());
            tracing::error!(
            ...
            if reconnect.auth_retry {
                // Auth retries are paced by the client worker while it polls
                // the token callback. A second exponential backoff here can
                // leave an already refreshed credential idle for 15 seconds.
    ...
    async fn work(
        ...
        .await?;
        // One client owns one session ID and advances the connection count for
        // every socket that opens, including client-requested reconnects.
