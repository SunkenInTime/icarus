...
- [x] Collapse the two stacked sort selects into one compact row (or move sorting into
      the content header)
- [x] Remove the permanently-disabled "Cloud Tools" buttons in cloud mode
      (`Import .ica` / `Import Backup` / `Export Library` render disabled, ~120px dead space)
- [x] Merge the duplicate Home entries in cloud mode ("Views → Home" vs root "Home" in Folders)
- [x] Chevron expand/collapse for nested folders (tree is currently always fully expanded),
      150–180ms easeOutCubic reveal
- [x] Row hover polish per main's vocabulary; drop the 500ms hover-exit timer on the "…" menu

## 3. Sync safety surfacing (trust-critical)

- [x] Persistent sync-status chip in the editor top strip driven by
...
- [x] Wire up conflict surfacing: `strategyConflictProvider` is written
      (`strategy_page_session_provider.dart:718`) but watched by nothing — conflicts
      resolve silently
- [x] Offline feedback ("working offline, will sync") — currently detected
      (`strategy_op_queue_provider.dart:371`) with zero UI
- [x] Persistent indicator for failed media uploads (today: one toast on first failure,
      then invisible; ops silently dropped after 8 attempts)

## 4. Library loading / empty / error states + workspace transitions

- [x] Skeleton grid while cloud lists load — `valueOrNull ?? []` currently shows
      "No cloud strategies yet" *during fetch* (`folder_content.dart:52`)
- [x] Real error state with retry (auth/network failures currently yield empty lists
      that read as "you have no strategies", `remote_library_provider.dart:74-94`)
- [x] Cloud-unavailable state to match the community placeholder's polish
      (icon + composition, `folder_content.dart:351-373`)
- [x] 200–250ms cross-fade when switching Local ↔ Cloud workspace (currently instant)
- [x] Disabled cloud rail items: opacity decay + transition when cloud becomes available

## 5. Cloud & role visibility in library and editor

- [x] Cloud badge on cloud strategy tiles (currently identical to local,
      `folder_content.dart:249-263`)
- [x] Owner/editor/viewer role badges on shared items in the library
- [x] Role visibility in the editor (e.g. "View only" chip) — today role silently
      disables buttons with no explanation

## 6. Auth polish + Account settings

- [x] Replace Material `AlertDialog` auth-incident prompt with ShadDialog
      (`auth_provider.dart:1137-1163`)
- [x] Humanize error copy — no "Convex"/"Supabase"/"session" jargon
      (`auth_provider.dart:557, 576, 817, 1140`)
- [x] Auth dialog: animated error presentation, input error borders, validation before
      submit-wait (`auth_dialog.dart:140-150`)
- [x] OAuth: don't pop the dialog blind — show a pending state during the Discord round-trip
      (`auth_dialog.dart:179-187`)
- [x] Sign-out feedback (toast) + loading affordance on the account rail item
- [x] Account section in settings: email, sign out, cloud connection status
      (none exists today)
- [x] Forgot-password / resend-confirmation affordances

## 7. Hardcoded color / theme sweep

- [x] `strategy_tile_sections.dart:93-97` — `Colors.redAccent`/`lightBlueAccent`/
      `orangeAccent` for attack/defend labels → tactical palette
- [x] `strategy_tile_sections.dart:284` — `Colors.deepPurpleAccent` drag border
- [x] `strategy_tile.dart:198-200` — `Colors.redAccent` delete → `destructive`
- [x] `folder_content.dart:276` — `Colors.grey` → `mutedForeground`
- [x] `folder_navigator.dart:352-371` — `Colors.white` in popover buttons → theme foreground
