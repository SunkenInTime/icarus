...
void main() {
  final fixture = jsonDecode(
      File('test/fixtures/abyss_tower_opening_2026_09_14.json')
          .readAsStringSync()) as Map<String, dynamic>;
  Offset point(dynamic xy) =>
      Offset((xy[0] as num).toDouble(), (xy[1] as num).toDouble());
  for (final side in ['attack', 'defense']) {
    test('Abyss $side tower opening preserves the surrounding wall', () {
      final directory =
          const bool.fromEnvironment('ICARUS_VERIFY_BUNDLED_GAMEPLAY')
              ? null
              : Platform.environment['ICARUS_ABYSS_TOWER_OPENING_MODELS'];
      final path = directory == null
          ? 'assets/maps/abyss_svg_height_$side.json.gz'
