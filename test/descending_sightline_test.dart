...
void main() {
  ...
  test('a box observer cannot see the ground behind a wall they look over', () {
    ...
    expect(eastRay(high, range: 110, supportId: 'box')?.wallId, 'tall');
    expectIntervals(
        east(high, supportId: 'box', range: 500), [(0, 35), (300, 500)]);
  });
  ...
  test('ground far enough behind a lower wall comes back into view', () {
    ...
    expect(eastRay(medium, range: 110, supportId: 'box'), isNull);
    expect(eastRay(medium, range: 45, supportId: 'box')?.wallId, 'mid');
    expectIntervals(
        east(medium, supportId: 'box', range: 500), [(0, 35), (140 / 3, 500)]);
  });
  ...
  test('a taller wall behind a low one hides everything between them', () {
    ...
    expectIntervals(east(pair, supportId: 'box'), [(0, 35)]);
    expect(eastRay(pair, range: 40, supportId: 'box')?.wallId, 'low');
  ...
  test('a wall beyond the box still hides the ground right behind it', () {
    final both = model([
      ...
      wall('crate', rectangle(20, -100, 21, 100), top: 2),
    ], supports: [
      support('box', rectangle(-5, -5, 5, 5), 3)
    ]);
    // 20 * 3 / 2.75 = 21.82 units.
    expectIntervals(east(both, supportId: 'box'), [(0, 20), (240 / 11, 100)]);
    ...
    expect(path.contains(const Offset(21, 0)), isFalse,
  ...
  test('a drop-off edge below the observer is their own floor, not a wall', () {
    ...
    expectIntervals(east(ledge, origin: origin), [(0, 50), (600 / 11, 100)]);
  });
}
