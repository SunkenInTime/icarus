...
void main() {
  final fixture = jsonDecode(
      File('test/fixtures/pearl_ramp_brick_profile_2026_09_14.json')
          .readAsStringSync()) as Map<String, dynamic>;
  Offset point(dynamic xy) =>
      Offset((xy[0] as num).toDouble(), (xy[1] as num).toDouble());
  for (final side in ['attack', 'defense']) {
    test('Pearl $side ramp brick sections use their complete source caps', () {
      final directory =
          const bool.fromEnvironment('ICARUS_VERIFY_BUNDLED_GAMEPLAY')
              ? null
              : Platform.environment['ICARUS_PEARL_RAMP_BRICK_PROFILE_MODELS'];
