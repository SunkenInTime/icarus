...
void main() {
  final fixture = jsonDecode(
      File('test/fixtures/lotus_low_crate_2026_09_14.json')
          .readAsStringSync()) as Map<String, dynamic>;
  Offset point(dynamic xy) =>
      Offset((xy[0] as num).toDouble(), (xy[1] as num).toDouble());
  for (final side in ['attack', 'defense']) {
    test('Lotus $side crate sides use their own source heights', () {
      final directory =
          const bool.fromEnvironment('ICARUS_VERIFY_BUNDLED_GAMEPLAY')
              ? null
              : Platform.environment['ICARUS_LOTUS_LOW_CRATE_MODELS'];
      final path = directory == null
          ? 'assets/maps/lotus_svg_height_$side.json.gz'
      ...
      for (final raw in fixture['floorControls'] ?? []) {
        final origin = point(raw['sides'][side]);
        ...
        expect(floor, closeTo(raw['expectedFloorMeters'] as num, .02),
            reason: raw['id']);
      }
      for (final raw in fixture['cases'] as List) {
        ...
        expect(defaultFloor,
            ...
            reason:
                '$label default uses the independently measured local floor');
        ...
        final cone = model.cone(
            ...
            supportId: selected?.id);
        final visiblePath = row['projected'] == true
            ? (cone.visibilityPath ?? (Path()..addPolygon(cone.polygon, true)))
            : (Path()..addPolygon(cone.polygon, true));
        expect(visiblePath.contains(target), row['visible'],
            reason: '$label production polygon');
      }
    });
  }
}
