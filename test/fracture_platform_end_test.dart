...
void main() {
  final fixture = jsonDecode(File('test/fixtures/fracture_platform_end_2026_09_15.json')
      .readAsStringSync()) as Map<String, dynamic>;
  Offset point(dynamic xy) =>
      Offset((xy[0] as num).toDouble(), (xy[1] as num).toDouble());
  for (final side in ['attack', 'defense']) {
    test('Fracture $side platform end clears while the barrier and column remain solid', () {
      final directory =
          const bool.fromEnvironment('ICARUS_VERIFY_BUNDLED_GAMEPLAY')
              ? null
              : Platform.environment['ICARUS_FRACTURE_PLATFORM_MODELS'];
