...
/// Location installed by the desktop build next to the application.
String heightNativeLibraryPath() {
  ...
  throw UnsupportedError('Standing visibility requires a desktop build.');
}
...
class HeightNativeWorker {
  ...
  /// Used by lifecycle tests without depending on a platform library.
  ...
  Future<HeightFrame> compute(int stamp, Float64List queries) async {
    if (_closed || _busy || _closing != null)
  ...
  Future<void> _close() async {
    // A map can be disposed while its final query is running. Let that query
    // release its lease before joining the native pool.
    await _idle?.future;
    if (_closed) return;
    _closed = true;
    _port.send(null);
