...
R=Path('E:/IcarusWorldAudit/2026-09-06/tactical-visibility-revision');after=R/'tower-contact-v15-local';M=json.loads((after/'manifest.json').read_text());w=json.loads(gzip.decompress(Path(M['displayWarpFile']).read_bytes()));source=np.array(w['sourceNativeMeters']).reshape(-1,2);target=np.array(w['targetAttackSvg']).reshape(-1,2);idx=np.array(w['triangles']).reshape(-1,3);tri=IndexedTriangles(source,idx);cases={c['id']:c for c in json.loads((R/'gallery-tower-v15-local/split-fixtures.json').read_text())['cases']};out=R/'tower-v14-v15-local-evidence';out.mkdir(exist_ok=True);font=ImageFont.truetype('C:/Windows/Fonts/segoeui.ttf',14);small=ImageFont.truetype('C:/Windows/Fonts/segoeui.ttf',12);records=[]
for row in M['cases']:
 id,side=row['id'],row['side'];before=R/('tower-diagonal84-contact-v14' if id.startswith('diagonal') else 'tower-contact-v14');bm=json.loads((before/'manifest.json').read_text());br=next(r for r in bm['cases'] if r['id']==id and r['side']==side);assert br['query']==row['query'];c=cases[id];center=np.array(c.get('targetSvg',c.get('sourceDirectedTargetSvg',np.mean(c['expectedAuthoredSpan'],axis=0))));p=np.array(row['query'][:2]);cell=tri.find_simplex(p[None])[0];t=tri.transform[cell];uv=t[:2]@(p-t[2]);origin=np.r_[uv,1-uv.sum()]@target[idx[cell]]
 for kind,rect in [('focus',np.array([center-[14,11],center+[14,11]])),('context',np.array([[300,140],[386,242]]))]:
  ...
  for i,(label,folder) in enumerate([('V14 connected tower',before),('V15 connected tower',after)]):
  ...
  d.text((16,8),f'Split / {side} / {id} / {kind} / native8x',font=font,fill='white');d.text((16,height+74),'Same physical source pose. Original pixels; no resampling.',font=small,fill='#d0d0d4');d.text((16,height+92),'Diagnostic candidate. Source profiles and neighboring attachments remain under review.',font=small,fill='#ffcc82');path=out/f'{side}-{id}-{kind}-native8x.png';im.save(path);a=np.array(Image.open(sources[0]['path']));b=np.array(Image.open(sources[1]['path']));records.append(dict(id=id,side=side,kind=kind,image=str(path),sourceImages=sources,sourceQuery=row['query'],originSvg=o.tolist(),cropPixels=list(map(int,box)),changedFullOverlayPixels=int(np.any(a!=b,axis=2).sum())))
