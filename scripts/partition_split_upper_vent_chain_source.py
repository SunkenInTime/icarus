...
out=REV/'split-upper-vent-chain-raw-partition-v2';out.mkdir(exist_ok=False)
fp=REV/'split-upper-vent-chain-field-proposal-v2/held-region-declaration.json';f=json.loads(fp.read_text())
...
protected={str(x['edge']):sorted(set(x['originalSourceFaces'])&set(map(int,raw_ids))) for x in prior['families'] if x['edge'] in [97,10099]}
...
report=dict(sourceGeometrySha256=sha(raw),declarationSha256=sha(fp),warpSha256=sha(wp),scriptSha256=sha(Path(__file__)),fragmentFileSha256=sha(out/'raw-source-fragments.npz'),
    ...
    elapsedSeconds=time.monotonic()-start,status='Held raw partition. Outside pieces preserve source XYZ. Original material admission and composed ray tests remain required.')
