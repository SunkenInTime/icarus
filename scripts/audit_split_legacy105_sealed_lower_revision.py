...
def main(declaration, out):
    ...
    families=json.loads(declaration.read_text());family,bottom=families;bottom_ids=np.array(bottom['reviewedSourceFaces']);selected_ids=np.unique(np.r_[family['reviewedSourceFaces'],bottom_ids]);bottom_set=set(bottom_ids.tolist())
    ...
    baseline_raw=full_correspondence[full_ids];bottom_baseline_faces=np.flatnonzero(np.isin(baseline_raw,bottom_ids));bottom_baseline=baseline.arrays['vertices'][baseline.arrays['faces'][bottom_baseline_faces]].copy();bottom_baseline[:,:,:2]=bottom_baseline[:,:,:2]@matrix.T+origin;bottom_baseline_ids=baseline_raw[bottom_baseline_faces];excluded=np.flatnonzero(np.isin(baseline_raw,selected_ids)).astype(np.int32)
    ...
    def mapped_sections(z):
        ...
        for line,index in zip(lines,ids):
            ...
            segment=shapely.LineString(line);is_bottom=raw_id in bottom_set;local_domain=shapely.box(*bottom['box']) if is_bottom else domain
            if not is_bottom:
            ...
            segment=segment.intersection(local_domain)
        ...
        old_lines,old_indices=sections(bottom_baseline,z)
        for line,index in zip(old_lines,old_indices):
            for part in shapely.get_parts(shapely.LineString(line).difference(shapely.box(*bottom['box']))):
                if part.geom_type=='LineString' and part.length>0:
                    p=np.array(part.coords);append(forward.apply(p),p,int(bottom_baseline_ids[index]),bottom_baseline[index],baseline,int(bottom_baseline_faces[index]))
        return result,np.array(owners),section_alpha
    ...
    eligible&=shapely.covers(receivers[0],shapely.points(shown));target_points=[[280,327.90],[288,327.90],[300,327.90],[310,327.90],[300,335],[310,335],[300,338],[288,307],[300,310.95],[310,310.95],[300,307],[310,307],[294,305],[314,327.9],[316,327.9],[320,327.9],[325,327.9],[330,327.9]]
    records=[];fixture_rows=[]
    for number,wanted in enumerate([[288,306],[296,316],[300,323],[280,323],[310,323],[310,339]]):
        ...
        start=np.r_[native[index],z];geometries,owners,section_alpha=mapped_sections(z);section_tree=shapely.STRtree(geometries)
    ...
    report=dict(scope=__doc__,declarationSha256=sha(declaration),sourceGeometrySha256=sha(raw_path),sourceNavigationSha256=sha(nav_path),baselinePackSha256=sha(complete/'split.height.bin.gz'),
        scriptSha256=sha(Path(__file__)),selectedMaterials='Selected masked sheets carry original-face UV endpoints through the source section and affine field, then use the independent alpha sampler. Outside bottom fragments retain their V29 oracle geometry and UVs. Other faces use the original independent alpha sampler.',
...
if __name__=='__main__':
    ...
    parser.add_argument('--declaration',type=Path,default=REV/'split-legacy105-connected-region-proposal-v6/combined-declarations.json')
    parser.add_argument('--out',type=Path,default=REV/'split-legacy105-lower-receiver-review-v6')
