...
def main(out=None, groups=None):
    out = out or REV / 'split-original-scene-connected-source-review-v1'
    out.mkdir(exist_ok=True)
    proposals = [json.loads(gzip.decompress((REV / f'split-connected-contour-proposals-v2/component-{c}.json.gz').read_bytes())) for c in [4, 6]]
    ...
    groups = groups or [
        ('iso-bottom-and-diagonal', [4580,4581,4582,4583,4584,4585,4586,5898,598,597,600,4381,501], [288,242,345,272], [7.25,8.25,9.75,11.75]),
        ('clove-mailroom-recess', [6953,6954,6957,6958,6961,6966,6840,7012,2192,6890,6891], [160,195,225,230], [8.25,9.75,11.75,14.75]),
        ('clove-upward-notch', [7609,7612,7479,7797,7790], [195,190,248,228], [7.25,8.25,9.25,11.75]),
    ]
    rows = []
    ...
    (out / 'source-context.json').write_text(json.dumps(dict(sourceGeometrySha256=hashlib.sha256(path.read_bytes()).hexdigest(),groups=rows,scope='Read-only complete source instances for the original Clove/Iso/Viper complaints. Finite plots do not authorize whole-object normalization.'),indent=2))
    print(out)
