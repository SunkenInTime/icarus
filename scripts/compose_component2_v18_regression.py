"""Native-pixel V17/V18 component2 seam regression evidence with unchanged physical queries."""
import json,gzip,hashlib
from pathlib import Path
import numpy as np
from PIL import Image,ImageDraw,ImageFont
from tactical_alignment_composite import IndexedTriangles
R=Path('E:/IcarusWorldAudit/2026-09-06/tactical-visibility-revision');before=R/'component2-contact-v17';after=R/'component2-contact-v18';output=R/'component2-v17-v18-regression-evidence';output.mkdir(exist_ok=True);versions=[('V17 prior shell',before),('V18 cover/body seam',after)];manifests=[json.loads((p/'manifest.json').read_text()) for _,p in versions];indices=[{(r['id'],r['side']):r for r in m['cases']} for m in manifests];w=json.loads(gzip.decompress(Path(manifests[1]['displayWarpFile']).read_bytes()));native=np.array(w['sourceNativeMeters']).reshape(-1,2);display=np.array(w['targetAttackSvg']).reshape(-1,2);cells=np.array(w['triangles']).reshape(-1,3);tri=IndexedTriangles(native,cells);cases={c['id']:c for c in json.loads((R/'gallery-component2-v18/split-fixtures.json').read_text())['cases']};proposal=json.loads(gzip.decompress((R/'split-connected-contour-proposals-v2/component-2.json.gz').read_bytes()));spans={s['completeSpan']:np.array(s['authoredEndpoints']) for s in proposal['spans']};font=ImageFont.truetype('C:/Windows/Fonts/segoeui.ttf',14);small=ImageFont.truetype('C:/Windows/Fonts/segoeui.ttf',12);records=[]
for key,row in indices[-1].items():
 ...
 if case.get('completeSpan') is not None:
  span=spans[case['completeSpan']];focus=np.array([span.min(0)-6,span.max(0)+6])
 else:focus=np.array([center-[10,10],center+[10,10]])
 ...
 for kind,rect in [('focus',focus),('context',np.array([[382,99],[435,145]]))]:
