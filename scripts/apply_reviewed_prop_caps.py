...
def compile_review(review_path, inputs, output):
    ...
    assert review['map'] in {'lotus', 'fracture', 'pearl'}
    stamp_key = review.get('stampKey', 'sourceLotusLowCrateReviewSha256')
    ...
    for side, path in inputs.items():
        ...
        for wall in before['walls']:
            domain = polygon(wall)
            parents = spec.get('parents', [spec['parent']])
            belongs = any(wall['id'] == parent or wall['id'].startswith(parent + '-')
    ...
    (output / 'reviewed-prop-cap-build.json').write_text(json.dumps(result, indent=2) + '\n')
