...
def main():
    out=REV/'split-legacy105-lower-receiver-review-v5'
    report_path=out/'first-hit-controls.json';report=json.loads(report_path.read_text())
    family_path=REV/'split-legacy105-connected-region-proposal-v5/region-declaration.json';family=json.loads(family_path.read_text());bottom=json.loads((family_path.parent/'bottom-continuation-declaration.json').read_text());selected_ids=np.unique(np.r_[family['reviewedSourceFaces'],bottom['reviewedSourceFaces']]);bottom_set=set(bottom['reviewedSourceFaces'])
    ...
    raw=np.load(ROOT/'supplemented-v2/world/split/geometry.npz');points=raw['points'];faces=raw['faces'];q=points[faces[selected_ids]].copy();q[:,:,:2]=q[:,:,:2]@matrix.T+origin
    def transform(p,side):
        p=np.asarray(p,dtype=float)
        return p if side=='attack' else ((p-origin)@inverse.T)@dm[:,:2].T+dm[:,2]
    def lines(z):
        source,indices=sections(q,z);result=[]
        for line,index in zip(source,indices):
            local_domain=shapely.box(*bottom["box"]) if int(selected_ids[index]) in bottom_set else domain
            segment=shapely.LineString(line).intersection(local_domain)
    ...
    (out/'native8x-context-manifest.json').write_text(json.dumps(dict(sourceReportSha256=sha(report_path),declarationSha256=sha(family_path),outputs=outputs,
        legend='Original artwork; thin green=proposed original-height source section bounds (masked alpha is applied to first-hit controls, not these outlines), violet=physical ray projected throughW, white=nav standing origin, red=first hit. These are source-diagnostic overlays, not an app renderer capture.'),indent=2)+'\n')
