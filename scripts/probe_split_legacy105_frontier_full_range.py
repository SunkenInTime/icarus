"""Full-range replay superseding the short-target104 source-role inference."""
import gzip
import hashlib
import json
...
def main():
    out=REV/'split-legacy105-frontier-review-v2'
    ...
    def cast(name,start,end):
        delta=end-start; delta=delta/np.linalg.norm(delta)
        end=start+delta*20.
    ...
    report=dict(scope=__doc__,physicalRangeMeters=20.,supersedes='v1 short targets at y280.95 stopped before source wall7899 at y280.87975. Its five-object source packet omitted that directly attached assembly. This replay uses complete packed source and retains all first-hit ownership.',sourceNavigationSha256=hashlib.sha256(nav_path.read_bytes()).hexdigest(),
