...
def main():
    out = REV / 'split-component7-remaining-assembly-review-v1'
    out.mkdir(exist_ok=True)
    proposals = [json.loads(gzip.decompress((REV / f'split-connected-contour-proposals-v2/component-{c}.json.gz').read_bytes())) for c in [6,7]]
    ...
    groups = [('hostel-attached-frames',[6726,6727,7205,6719,6720],[84,102,122,143],[5.75,7.75,9.75,11.75]),('tower-attached-doorframe',[6946,6944],[168,128,181,162],[5.75,9.75,11.75,15.75]),('balcony-connected-other-unit',[6946,6947,6961],[140,151,185,185],[5.75,9.75,11.75,15.75])]
    rows = []
    ...
    (out / 'source-context.json').write_text(json.dumps(dict(sourceGeometrySha256=hashlib.sha256(path.read_bytes()).hexdigest(),groups=rows,scope='Read-only omitted attached frames and shared balcony unit at component7/6. Finite plots do not authorize whole-object normalization.'),indent=2))
    print(out)
