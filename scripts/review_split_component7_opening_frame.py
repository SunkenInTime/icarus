...
def main():
    out = REV / 'split-component7-opening-frame-review-v1'
    ...
    groups = [('tower-opening-frame-and-upper-attachments',[6946,6948,6831,6962,6965,2246,2249],[123,175,147,212],[6.75,9.75,12.75,16.75])]
    rows = []
    ...
    (out / 'source-context.json').write_text(json.dumps(dict(sourceGeometrySha256=hashlib.sha256(path.read_bytes()).hexdigest(),groups=rows,scope='Read-only source opening-frame199 and separate upper window/scaffold profiles. Finite plots do not authorize whole-object normalization.'),indent=2))
    print(out)
