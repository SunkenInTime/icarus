"""Compare an isolated ordered half-plane cache with the same frozen cone policy."""
...
def run(output):
    ...
    for row in frozen['records']:
        ...
        if worst is None or before['nativeSeconds']>worst[0]:worst=(before['nativeSeconds'],row,origin,seeds,reference['mesh'])
